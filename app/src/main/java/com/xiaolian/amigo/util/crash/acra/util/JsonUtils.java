...
public final class JsonUtils {
    ...
    public static JSONObject toJson(CrashReportData data, Map<ReportField, String> fieldMap) {
        Map<String, Object> map = new HashMap<String, Object>();
        for (Map.Entry<ReportField, Element> entry : data.entrySet()) {
            Element element = entry.getValue();
            if (fieldMap == null || fieldMap.get(entry.getKey()) == null) {
                map.put(entry.getKey().name(), element.value());
            } else {
                map.put(fieldMap.get(entry.getKey()), element.value());
            }
        }
        return new JSONObject(map);
    }
