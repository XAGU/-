...
public class RxBle {
    ...
    private RxBle() {
        mBus = PublishSubject.<String>create().toSerialized();
//        mBus = new SerializedSubject<>(PublishSubject.<String>create());
    ...
    public void scanBleDevices(boolean enable) {
        if (enable) {
            ...
            Observable.timer(SCAN_PERIOD, TimeUnit.MILLISECONDS).subscribe(new Consumer<Long>() {
                @Override
    ...
    private class BleGattCallback extends BluetoothGattCallback {
        ...
        public void onServicesDiscovered(final BluetoothGatt bleGatt, int status) {
            ...
            Observable.fromIterable(serviceList)
                    .flatMap(new Function<BluetoothGattService, Observable<BluetoothGattCharacteristic>>() {
                        @Override
                        public Observable<BluetoothGattCharacteristic> apply(BluetoothGattService bleGattService) {
                            return Observable.fromIterable(bleGattService.getCharacteristics());
                        }
                    })
                    .filter(new Predicate<BluetoothGattCharacteristic>() {
                        @Override
                        public boolean test(BluetoothGattCharacteristic bleGattChar) {
                            return bleGattChar.getUuid().toString().equals(UUID);
                        }
                    })
                    .subscribe(new Consumer<BluetoothGattCharacteristic>() {
                        @Override
                        public void accept(BluetoothGattCharacteristic bleGattChar) {
